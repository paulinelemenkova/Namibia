...
\begin{abstract}
Identification of fluctuations in Etosha salt pan poses environmental challenges. Etosha Pan presents unique salt landscape  in northern Namibia. With seasonal fluctuation cycles lead to water level in the basin contrasts from evaporated to flooding. Monitoring landscapes dynamics is possible using a time series analysis of the remote sensing (RS) data. Image classification is the most common approach to map changes in land cover types. Recent development in methods of Machine Learning (ML) enabled their applications in Earth data processing. Ensemble learning approach with Random Forest (RF) presents the example of such methods applied to image classification. Based on integrating several ML approaches, it includes the decision trees algorithm with controlled variance that improves the accuracy of image partition. This paper evaluates image classification methods using training algorithm of RF by GRASS GIS including the embedded Scikit-Learn ML library of Python. Several modules of GRASS GIS were investigated and tested using a combination of 'r.learn.predict', 'r.random', 'r.learn.train', 'r.category', 'i.group'. Land cover types around Etosha Pan were identified using classification of six multi-spectral Landsat 8-9 OLI/TIRS images collected in different seasons. This enabled to monitor the process of water evaporation in Etosha Pan during the transitional wet-to-dry period. The comparison of images revealed variations in the surface of basin from wet period in March to the dry period in August when the basin is covered by the crust of salt and minerals. The series of maps made using GRASS GIS scripts demonstrated the opportunities of the ensemble learning tools to image processing by RF classifiers. Such novel approaches enable accurate image classification for environmental analysis of landscape dynamics. The proposed method of ML-based image classification in GRASS GIS and its efficacy in RS data evaluation proposes an accurate tool for forecasting and prognosis of changes in landscapes of Namibia.
\end{abstract}
